...
A graph G is called k-connected if for every ${\displaystyle S\subseteq V(G)}$ 
, and ${\displaystyle |S|<k} $
, 
G − S {\displaystyle G-S} 
 is connected, and ${\displaystyle |G|>k}$. 
Similarly, a graph G is called $ℓ {\displaystyle \ell } $
...
δ ( G ) {\displaystyle \delta (G)} 
 edge-connected, and so ${\displaystyle \lambda (G)\leq \delta (G)}$. 
